...
namespace FrameProtocol
{
    public class FrameProtocol
    {
        ...
        public FrameProtocol(IDuplexPipe pipe)
        {
            _reader = pipe.Input;
            _writer = pipe.Output;
        }

        public FrameProtocol(PipeReader reader, PipeWriter writer)
        {
            _reader = reader;
            _writer = writer;
        }
        ...
        public bool TryReadPacketBodyLen(in ReadOnlySequence<byte> buffer, out uint packetLength)
        {
        ...
        private static void ThrowFrameSizeEx()
        {
